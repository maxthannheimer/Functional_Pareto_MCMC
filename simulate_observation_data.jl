...
param_start=[gaussian_proposal_cut(1.0,1.5,500.0),rand()*2.0]
alpha_start=gaussian_proposal_cut(1.0,1.0,50.0)
...
#function to simulate observations on a 7x7 grid, with fixed 9 coarse observation points
function simulate_observations_fixed_9x9(gridsize,N_coarse,num_sim,num_runs,beta_true,c_true, alpha_true,path)
#create grid and observation points, also plots if last argument is true
#here the coarse observation points are already on the fine grid
#also safe coords 
(coord_coarse, coord_fine, row_x0)=Create_Grid_and_Observation_9x9(false)
